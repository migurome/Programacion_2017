...
void nuevaListaCadenas(tListaCadenas &listaCadenas){
	listaCadenas.tam = 0;
	listaCadenas.capacidad = CADENAS;
	listaCadenas.arrayCadena = new string[listaCadenas.capacidad];
}

void liberarListaCadenas(tListaCadenas & listaCadenas){
	delete[] listaCadenas.arrayCadena;
	listaCadenas.tam = 0;
	listaCadenas.arrayCadena = nullptr;
}

void ampliar(tListaCadenas &listaCadenas){
	string *aux = new string[(listaCadenas.capacidad *3)/2 + 1];
	for (int i = 0; i < listaCadenas.tam;i++){
		aux[i] = listaCadenas.arrayCadena[i];
	}
	delete[] listaCadenas.arrayCadena;
	listaCadenas.arrayCadena = aux;//copia la direccion de aux
	listaCadenas.capacidad = (listaCadenas.capacidad * 3)/2  + 1;
	aux = nullptr;
}
